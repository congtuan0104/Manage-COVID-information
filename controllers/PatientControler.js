...
class PatientControler {
  ...
  async home(req, res, next) {
    ...
    const notifications = await db.getNotification(1);
    // }
    res.render("home", {
      layout: "userLayout",
      title: "Trang chủ",
  ...
  async buyPackage(req, res) {
    ...
    const result = await db.addPackage(
      order_id,
      patientID,
      timeOrder,
      packageID,
    ...
    var isPackageConsumtionExists = await db.isExistPackageConsumption(
      packageID,
      newDate
    );
    ...
    suppliesofPackage.forEach(async (supply, index) => {
      let res = await db.addOrderDetail(
        order_id,
        supply.supplies_id,
        nProduct[index],
        supply.price * nProduct[index]
      );
    });
    if (result == 0) {
