...
module.exports = {
  ...
  getRemainingPackage: async (packageID) => {
    const res = await db.any(
      `SELECT * from supplies where supplies_id not in
  ...
  getOrderList: async (patient_id) => {
    const res = await db.any(
      "SELECT *, TO_CHAR(timeorder,'dd/mm/yyyy hh:mm:ss') AS timeorder FROM orders WHERE patient_id = $1",
      [patient_id]
    );
    return res;
  },
  getOrderDetailByOrderId: async (order_id) => {
    const res = await db.any("SELECT *FROM order_detail WHERE order_id = $1", [
      order_id,
    ]);
    return res;
  },
  getOrderListDetail: async (patient_id) => {
    const res = await db.any(
      "SELECT *, TO_CHAR(timeorder,'dd/mm/yyyy hh:mm:ss') AS timeorder FROM orders,package,patient WHERE orders.patient_id = $1 AND orders.patient_id = patient.patient_id AND package.package_id = orders.package_id",
      [patient_id]
    );
    return res;
  },
  ...
  updatePackageConsumption: async (package_id, date) => {
    await db.none(
      `UPDATE package_consumption SET consume = consume+1 WHERE package_id = $1 AND date = $2`,
      [package_id, date]
    );
  },
  isExistPackageConsumption: async (package_id, date) => {
    const res = await db.any(
      "SELECT * FROM package_consumption WHERE package_id=$1 AND date = $2",
      [package_id, date]
    );
  ...
  getOrderListByIdByDate: async (packageID) => {
    ...
    const res = await db.any(
      `SELECT * from orders where package_id=$1 and timeorder >=$2 and timeorder <=$3`,
      [packageID, lastDateStr, currentDate]
    );
    if (res.length == 0) return null;
    return res;
  },
  ...
  getListTreatmentHistoryById: async (patient_id) => {
    const res = await db.any(
      "SELECT *,TO_CHAR(update_time,'dd/mm/yyyy hh:mm:ss') AS update_time FROM treatment_history,treatment_place WHERE treatment_history.patient_id=$1 AND treatment_history.place_id = treatment_place.place_id",
      [patient_id]
    );
    if (res.length == 0) return null;
    return res;
  ...
  getNotification: async (patient_id) => {
    const res = dbModel.get(patient_id, "notification", "patient_id");
    return res;
  },
};
